...
WSRESTFUL taxinvoices DESCRIPTION "Lista e Inclui clientes no ERP Protheus "
	...
	WSMETHOD GET    DESCRIPTION 'Retorna lista de clientes' WSSYNTAX '/taxinvoices' /*PATH 'taxinvoices'*/ PRODUCES APPLICATION_JSON
	WSMETHOD POST   DESCRIPTION 'Inclusão de registro'      WSSYNTAX '/taxinvoices' /*PATH 'taxinvoices'*/ PRODUCES APPLICATION_JSON
	WSMETHOD PUT    DESCRIPTION 'Exclusão de registro'      WSSYNTAX '/taxinvoices' /*PATH 'taxinvoices'*/ PRODUCES APPLICATION_JSON

END WSRESTFUL

//-------------------------------------------------------------------
/*/{Protheus.doc} GET / taxinvoices
...
WSMETHOD GET WSRECEIVE searchKey, page, pageSize, endDate, startDate, days WSSERVICE taxinvoices
    ...
    If !Empty(cSearchKey)
        ...
        /* --- Monta filtro multi?palavra para o nome --- */
        For nPos := 1 To Len(cSearch + " ")
            If SubStr(cSearch + " ", nPos, 1) == " "
                If !Empty(cToken)
                    aAdd(aWords, cToken)
                    cToken := ""
                EndIf
            Else
                cToken += SubStr(cSearch, nPos, 1)
            EndIf
        Next

        If Len(aWords) > 0
            cNameWords := "("
            For nPos := 1 To Len(aWords)
                cNameWords += " SA1.A1_NOME LIKE '%" + aWords[nPos] + "%'"
                If nPos < Len(aWords)
                    cNameWords += " AND"
                EndIf
            Next
        ...
        // SUBSTITUI A LINHA ORIGINAL POR ESTE BLOCO:
        If !Empty(cNameWords)
            cWhereSF2 += cNameWords + " OR "
        Else
            // fallback se não gerou palavras (não deve ocorrer)
