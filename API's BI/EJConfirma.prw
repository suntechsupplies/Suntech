...
WSMETHOD POST WSSERVICE EJConfirma
	...
	ElseIf oObj:CONFIRMACAO[1]:TABELA == 'SE1'
		...
		For i:=1 To Len(oObj:CONFIRMACAO[1]:DADOS)
			...
			_cFilial := oObj:CONFIRMACAO[1]:DADOS[i]:E1_FILIAL 
			cPrefixo := oObj:CONFIRMACAO[1]:DADOS[i]:E1_PREFIXO
			cNum     := oObj:CONFIRMACAO[1]:DADOS[i]:E1_NUM
			cParcela := oObj:CONFIRMACAO[1]:DADOS[i]:E1_PARCELA
			cTipo    := oObj:CONFIRMACAO[1]:DADOS[i]:E1_TIPO
