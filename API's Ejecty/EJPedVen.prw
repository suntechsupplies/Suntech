...
WSMETHOD GET WSRECEIVE nPag, nFlagCab, nFlagIt, TenantId WSSERVICE EjPedGP
	...
	If FindFunction("WfPrepEnv")
		...
		BeginSql Alias cAliasTmp
			...
			SELECT (( DENSE_RANK() OVER ( ORDER BY C5_NUM) ) / 100 ) + 1 AS PAG,
			C5_FILIAL, C5_ZZNPEXT, C5_CLIENTE, C5_LOJACLI, C6_NOTA, C5_EMISSAO, C5_ZZORIGE, C5_ZZSITCO, C5_ZZSITFI, C5_ZZTPPED, C5_ZZDTEMI, C5_CONDPAG, C5_TRANSP, C5_TPFRETE, C5_VEND1, 
			C5_ZZCUPOM, C5_NUM, C6_PRODUTO, C6_DESCRI, C6_QTDVEN, C6_QTDVEN, C6_QTDENT, C6_DESCONT, C6_VALOR, B1_PESO, C6_ITEM,
			(
				SELECT SUM(C6_VALOR) FROM SC6010 AS SC6
				WHERE 0=0
				AND SC6.C6_FILIAL = SC6010.C6_FILIAL
				AND SC6.C6_NUM    = SC6010.C6_NUM
				AND SC6.C6_CLI    = SC6010.C6_CLI
				AND SC6.C6_LOJA   = SC6010.C6_LOJA
				AND SC6.D_E_L_E_T_ = ''
			) AS C6_VLRPED,
			(
				SELECT SUM(D2_TOTAL) AS D2_TOTAL FROM SC6010, SD2010 
				WHERE 0=0
				AND SC6010.C6_FILIAL =  SD2010.D2_FILIAL
				AND SC6010.C6_NUM =  SD2010.D2_PEDIDO 
				AND SC6010.C6_ITEM =  SD2010.D2_ITEM
				AND SC6010.C6_NUM = C5_NUM
			...
			AND SC5010.C5_ZSTATUS <> '9'
			ORDER BY PAG, C5_NUM, C6_NOTA, C6_ITEM
				...
				While (cAliasTmp)->( !Eof() )
					...
					IF (cAliasTmp)->PAG = nPag
						...
						If TRIM( (cAliasTmp)->C5_FILIAL + (cAliasTmp)->C5_NUM ) <>  cPed
							...
							cSetResp  += '","NOTAFISCAL":"'		    + TRIM((cAliasTmp)->C6_NOTA)
							cSetResp  += '","CODIGOCLIENTE":"'		+ TRIM((cAliasTmp)->C5_CLIENTE + (cAliasTmp)->C5_LOJACLI)
							cSetResp  += '","STATUSPEDIDO":"'		+ TRIM(IIF(EMPTY(TRIM((cAliasTmp)->C6_NOTA)), "EM ABERTO", "FATURADO"))
							cSetResp  += '","DATAINTEGRACAO":"' 	+ TRIM((cAliasTmp)->C5_ZZDTEMI)
							cSetResp  += '","STATUSFINANCEIRO":"'	+ TRIM(cValToChar((cAliasTmp)->C5_ZZSITFI))
							cSetResp  += '","STATUSCOMERCIAL":"'	+ TRIM(cValToChar((cAliasTmp)->C5_ZZSITCO))
							cSetResp  += '","VALORPEDIDO":'			+ TRIM(cValToChar((cAliasTmp)->C6_VLRPED))
							cSetResp  += ',"VALORFATURADO":' 		+ TRIM(cValToChar((cAliasTmp)->D2_VLRFAT))
							...
							cSetResp  += '","TRANSP":"'				+ TRIM((cAliasTmp)->C5_TRANSP)
	...
	If lower(Self:GetHeader("Content-Type", .F.)) == "application/json"
		...
		If ValType(oContent) == "J"
			...
			For _nZ := 1 To Len(oContent["CABECALHO"])
				...
				aAdd(aDadosC5, {"C5_TRANSP" 		,Iif(ValType(oContent["CABECALHO"][_nZ]["C5_TRANSP"])   =="U","",oContent["CABECALHO"][_nZ]["C5_TRANSP"]) 			    , Nil})
				...
				aAdd(aDadosC5, {"C5_ZZORIGE" 		,Iif(ValType(oContent["CABECALHO"][_nZ]["C5_ZZORIGE"])  =="U","",oContent["CABECALHO"][_nZ]["C5_ZZORIGE"])              , Nil})     //Alterado para gravar o campo C5_ZZORIGE pois estava fixo "B2B"													                                                                , Nil})
				...
				aAdd(aDadosC5, {"C5_ZZSITFI" 		,Iif(ValType(oContent["CABECALHO"][_nZ]["C5_ZZSITFI"])  =="U","",oContent["CABECALHO"][_nZ]["C5_ZZSITFI"])				, Nil})		// Incluido em 30/05/2025 por Antonio Ricardo de Araujo - Para atender a API B2B do Protheus
				aAdd(aDadosC5, {"C5_ZZSITCO" 		,Iif(ValType(oContent["CABECALHO"][_nZ]["C5_ZZSITCO"])  =="U","",oContent["CABECALHO"][_nZ]["C5_ZZSITCO"])				, Nil})		// Incluido em 30/05/2025 por Antonio Ricardo de Araujo - Para atender a API B2B do Protheus
