			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								IIF( SC6->(FieldPos("C6_CODISS"))>0, IIF( !Empty(SC6->C6_CODISS),AllTrim(SC6->C6_CODISS),AllTrim(SF3->F3_CODISS)),SF3->F3_CODISS),; //24 - Codigo Servico ISS
...
Static Function tomador( aDest )
	...
	If lTomador
		...
		EndIf
		cString += "<nomeFantasiaTom>" + allTrim( aDest[27] ) + "</nomeFantasiaTom>"
