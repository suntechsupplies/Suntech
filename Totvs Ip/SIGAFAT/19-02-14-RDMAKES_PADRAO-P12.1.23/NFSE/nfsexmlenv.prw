...
Static Function tomador( aDest )
	...
	If !lSemTomador 		
		...
		EndIf
		If Len( cCodTom ) <= 5 .And. (!(cCodTom $ '99999').Or. (cCodTom $ '99999' .And. if( type( "oSigamatX" ) == "U",SM0->M0_CODMUN,oSigamatX:M0_CODMUN ) $ '3550308|3552205|4205407|3300704|3156700|4115200|4208203'))
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cFntCtrb,aLeiTrp,lRecIrrf)	
	...
	for nX := 1 to len( aProd )
		...
		if (valtype(aProd[nX][29]) <> "U")
			if (valtype(aProd[nX][29]) == "N")
				if ( (aProd[nX][29]) > 0 .and. !empty(cCampoDiscDed) )
					if select("SC5") > 0 
