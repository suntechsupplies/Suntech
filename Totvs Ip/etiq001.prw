...
static function imprEtiq001()
	...
	for nI := 1 to nImpress
		...
		if Empty(cBarZZ2)
			msgAlert("Código de Barras não encontrado. Verifique tabela ZZ2!")
		else

			/* COLUNA, LINHA, TEXTO */

			MSCBSAYBAR(nColuna+10,nLinha,cBarZZ2,"N","MB07",10,.f.,.F.,.F.,,2,2,.t.)
			nLinha := nLinha+12
			if !Empty(aProduto[4])
			...
			if !Empty(aProduto[8])
				MSCBSAYBAR(nColuna+10,nLinha,aProduto[8],"N","MB04",10,.F.,.F.,.F.,,2,1,.F.,.F.,"1",.T.) // Código Barras EAN
				nLinha := nLinha+11
				cEAN := SUBSTR(aProduto[8], 1, 1)
				MSCBSAY(nColuna+8,nLinha,cEAN,"N","0","021",,,,,.t.) // Código EAN
				cEan := SUBSTR(aProduto[8], 2, 6)
				MSCBSAY(nColuna+11,nLinha,cEAN,"N","0","021",,,,,.t.) // Código EAN
				cEan := SUBSTR(aProduto[8], 8, 6)
				MSCBSAY(nColuna+23,nLinha,cEAN,"N","0","021",,,,,.t.) // Código EAN
			endif
...
Static Function ValidPerg()
	...
	aAdd(aParamBox,{1,"Código Produto ",space(getSX3Cache("B1_COD","X3_TAMANHO")),"","","SB1","",50,.T.}) //MV_PAR01
	aAdd(aParamBox,{1,"Quantidade de Etiquetas",0,"@E 9,999","mv_par02>0","","",20,.F.}) // MV_PAR02

	If ParamBox(aParamBox,"Etiqueta Produto",@aRet,,,,,,,"etiq001",.T.,.T.)
		lRet := .t.
	EndIf
