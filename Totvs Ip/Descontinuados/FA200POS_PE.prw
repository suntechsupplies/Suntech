...
static Function VldRet()
	...
	If (cBanco == "341" .and. Alltrim(cOcorr) == "04") //Itaú
		...
		If SE1->(DbSeek(Substr(cNumTit,1,10))) //IdCnab
			...
			SEA->EA_AGEDEP  := cAgencia //Aqui
			SEA->EA_NUMCON  := cConta   //Aqui
			//Esse Campo (SEA->EA_NUMBOR  := cBordero) foi adicionado para gravar, uma vez que não utiliza-se na empresa a geração de Bordero apenas de alteração
			//Quando o título sofria alteração e retornava do banco e a tabela SEA já não possuia o numero do Bordero original o registro não era excluido
			//antes da gravação, gerando assim errorlog de duplicidade na tabela SEA. - Suntech - Ricardo Araujo -- 03/05/2022
			SEA->EA_NUMBOR  := cBordero 
			SEA->EA_DATABOR	:= dBaixa
			SEA->EA_CART  	:= "R"
			SEA->EA_SITUACA := "1"      //Aqui
