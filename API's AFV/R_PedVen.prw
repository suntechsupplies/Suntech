...
/*
user function r_PedVen()

return
*/
WSRESTFUL rPedGetPos DESCRIPTION Desc_Rest
	...
	WSMETHOD GET  DESCRIPTION Desc_Get  WSSYNTAX "/rPedGetPos || /rPedGetPos/{}"
	WSMETHOD POST DESCRIPTION Desc_Post WSSYNTAX "/rPedGetPos/{Pedido}"
...
WSMETHOD GET WSRECEIVE nPag, nFlagCab, nFlagIt, cTipo HEADERPARAM TENANTID WSSERVICE rPedGetPos
    ...
    */
	...
	If !Empty(SELF:TENANTID)
        ...
        CONOUT("[GET] [RPEDGETPOS - ACACIA] TENANTID : " + SELF:TENANTID + "[cEmpant] : " + cEmpant + "[cFilant] : " + cFilant)
	EndIf
			...
			While (cAliasTmp)->( !Eof() )
				...
				IF (cAliasTmp)->PAG = nPag
					...
					If (cAliasTMP)->C5_STATU1 <> '9'
						SC5->(dbSetOrder(1))
						SC5->(dbGoTop())
						If SC5->(dbSeek((cAliasTMP)->(C5_FILIAL + C5_NUM)))
							RecLock("SC5",.F.)
							SC5->C5_STATU1 := '9'
							SC5->(MsUnlock())
						Endif
			...
			While (cAliasTmp)->( !Eof() )
				...
				IF (cAliasTmp)->PAG = nPag
					...
					If (cAliasTMP)->C6_STATU1 <> '9'
						SC6->(dbSetOrder(1))
						SC6->(dbGoTop())
						If SC6->(dbSeek((cAliasTMP)->(C6_FILIAL+C6_NUM+C6_ITEM+C6_PRODUTO)))
							RecLock("SC6",.F.)
							SC6->C6_STATU1 := '9'
							SC6->(MsUnlock())
						Endif
...
WSMETHOD POST HEADERPARAM TENANTID WSSERVICE rPedGetPos
	...
	Local cError				:= ''
 	...
 	Local _cNumPed				:= ''
	Local cAliasTMP             := GetNextAlias()
    //Local aTabs					:= {'SC5','SC6','SA1','SA2','SB1','SB2','SE4','SF4','SC5'}
	...
	If !Empty(SELF:TENANTID) .Or. ! ValType(self:tenantid) == "U"
        ...
        cFilAnt := _cFilial
        cNumemp := _cEmpresa + _cFilial
        CONOUT("[POST] [RPEDGETPOS - ACACIA] [ TENANTID : " + iif(ValType(SELF:TENANTID)=="U","Indefined", SELF:TENANTID) + " ][cEmpant : " + cEmpant + " ][cFilant : " + cFilant +" ][ cNumemp : " + cNumemp )
    Else
        CONOUT("[POST] [RPEDGETPOS - ACACIA] [ TENANTID : " + iif(ValType(SELF:TENANTID)=="U","Indefined", SELF:TENANTID) + " ][cEmpant : " + cEmpant + " ][cFilant : " + cFilant +" ][ cNumemp : " + cNumemp )
	EndIf
	...
	If lower(Self:GetHeader("Content-Type", .F.)) == "application/json"
        ...
        Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Content-Type : '+ Self:GetHeader("Content-Type", .F.) + ']')
		...
		If ValType(oContent) == "J"
            ...
            For _nZ := 1 To Len(oContent["CABECALHO"])
                ...
                Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] - [Quantidade de Pedidos no Consumo : ' + Alltrim(cValToChar(Len(oContent["CABECALHO"]))) + " ] Importação Iniciada ......")            
                ...
                If !Empty(SELF:TENANTID)
                    ...
                    CONOUT("[POST] [RPEDGETPOS - ACACIA] [ TENANTID : " + SELF:TENANTID + "[cEmpant] : " + cEmpant + "[cFilant] : " + cFilant)
                Else
                    CONOUT("[POST] [RPEDGETPOS - ACACIA] [ TENANTID : Sem conteúdo ] [cEmpant] : " + cEmpant + "[cFilant] : " + cFilant)
                EndIf
                ...
                CONOUT("[POST] [RPEDGETPOS - ACACIA] [ Array de Cabecalho do Pedido de Vendas preenchido com sucesso ]")
                ...
                CONOUT("[POST] [RPEDGETPOS - ACACIA] [ " + Alltrim(Str(Len(oContent["CABECALHO"][_nZ]["ITENS"]))) + " ítens Array dos Itens do Pedido de Vendas preenchido com sucesso ]")
                ...
                //**********************************************************************************
                If SC5->(dbSeek( PADR(_cFilial,2) + PADR(_cPedAFV,10)) )
                    LOOP
                Endif
                ...
                If lMsErroAuto	
                    ...
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : sucessMessage ] ' + EncodeUTF8(IIF(_nOpc == 3 ,"Pedido de Vendas não Incluído","Pedido de Vendas não Alterado")) )
                    
                    oJsonRet["LOJA"]			:= oContent["CABECALHO"][_nZ]["C5_LOJACLI"]
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : loja ] ' + oContent["CABECALHO"][_nZ]["C5_LOJACLI"] )

                    oJsonRet["EMPRESA"]			:= oContent["CABECALHO"][_nZ]["EMPRESA"]
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : empresa ] ' +  oContent["CABECALHO"][_nZ]["EMPRESA"])
                    
                    oJsonRet["PEDIDOAFV"]		:= oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : pedidoAFV ] ' + oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"])

                    oJsonRet["PEDIDOPROTHEUS"]	:= _cNumPed 
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : pedidoProtheus ] ' + _cNumPed)

                    oJsonRet["ARQLOG"]			:= StrTran(EncodeUTF8(cError),'\r\n','')
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : arqLog ] ' + StrTran(EncodeUTF8(cError),'\r\n',''))

                    oJsonRet["FILIAL"]         	:= oContent["CABECALHO"][_nZ]["C5_FILIAL"]
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : filial ] ' + oContent["CABECALHO"][_nZ]["C5_FILIAL"])

                    oJsonRet["CLIENTE"]			:= oContent["CABECALHO"][_nZ]["C5_CLIENTE"]
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : cliente ] ' + oContent["CABECALHO"][_nZ]["C5_CLIENTE"])
                    
                    oJsonRet["OPCAO"]			:= EncodeUTF8(IIF(_nOpc == 3, "3 - Inclusão", "4 - Alteração"))
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : opcao ] ' + EncodeUTF8(IIF(_nOpc == 3, "3 - Inclusão", "4 - Alteração")))

                    oJsonRet["SUCESSCODE"]     	:= 202  // 202 - Código padrão HTML de POST recebido, porem nao processado
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : opcao ] 202' )
                ...
                Else
                    ...
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : sucessMessage ] ' + EncodeUTF8(IIF(_nOpc == 3 ,"Pedido de Vendas AFV incluído com sucesso","Pedido de Vendas AFV alterado com sucesso" )) )
                    
                    oJsonRet["LOJA"]			:= (cAliasTMP)->C5_LOJACLI							
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : loja ] ' + IIF(ValType((cAliasTMP)->C5_LOJACLI) == "U", "",(cAliasTMP)->C5_LOJACLI))
                    
                    oJsonRet["EMPRESA"]			:= cEmpant									
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : empresa ] '+ cEmpant )
                    
                    oJsonRet["PEDIDOAFV"]		:= _cPedAFV
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : pedidoAFV ] '+ _cPedAFV)
                    
                    oJsonRet["PEDIDOPROTHEUS"]	:= (cAliasTMP)->C5_NUM 
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : pedidoProtheus] '+ IIF(ValType((cAliasTMP)->C5_NUM) == "U", "",(cAliasTMP)->C5_NUM))
                    
                    oJsonRet["FILIAL"]         	:= (cAliasTMP)->C5_FILIAL								
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : filial ] '+ IIF(ValType((cAliasTMP)->C5_FILIAL) == "U", "",(cAliasTMP)->C5_FILIAL))
                    
                    oJsonRet["CLIENTE"]			:= (cAliasTMP)->C5_CLIENTE								
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : cliente ] '+ IIF(ValType((cAliasTMP)->C5_CLIENTE) == "U", "",(cAliasTMP)->C5_CLIENTE))
                    
                    oJsonRet["OPCAO"]			:= EncodeUTF8(IIF(_nOpc == 3, "3 - Inclusão", "4 - Alteração"))
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : opcao ] '+ EncodeUTF8(IIF(_nOpc == 3, "3 - Inclusão", "4 - Alteração")))
                    
                    oJsonRet["SUCESSCODE"]     	:= 201  				                        
                    Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Log de retorno AFV : sucessCode] 201' ) 
                ...
                Conout( '[RPedGetPos - Post] [Thread '+cValToChar(ThreadID())+ '] ['+ FWTimeStamp(2) +'] [Pedido Externo : '+oContent["CABECALHO"][_nZ]["C5_ZZNPEXT"]+'] [ Pedido Protheus : ]' + _cNumPed +' - Importação Finalizada. ' + iif(_nOpc == 3, "Pedido Incluido com sucesso !!!", "Pedido Alterado com sucesso !!!"))
