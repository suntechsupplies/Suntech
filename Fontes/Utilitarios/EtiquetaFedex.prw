...
Static Function ImpEtiq()
	...
	cQuery := "SELECT B1_COD , B1_DESC, B1_CODBAR FROM SB1010 WHERE B1_COD BETWEEN '" + MV_PAR01 + "' AND '" + MV_PAR02 + "' AND D_E_L_E_T_ <> '*'"
	...
	While (cAlias)->(!Eof())
		For nR := 1 to nQuant
			...
			MsProcTxt("Imprimindo " + (cAlias)->B1_COD + " - " + (cAlias)->B1_DESC + "...")
			...
			oPrinter:FWMSBAR("CODE128" , nLinC , nColC, alltrim((cAlias)->B1_CODBAR), oPrinter,/*lCheck*/,/*Color*/,/*lHorz*/, nWidth, nHeigth,.F.,/*cFont*/,/*cMode*/,.F./*lPrint*/,nPFWidth,nPFHeigth,lCmtr2Pix)

			nLin+= 40
			oPrinter:Say(nLin,nCol,alltrim((cAlias)->B1_COD) + " - " + alltrim((cAlias)->B1_DESC),oFont16)
