...
User Function zEnvMail(cDe, cPara, cCC, cAssunto, cCorpo, aAnexos, lMostraLog, lUsaTLS)
    Local aArea        := GetArea()
    Local nAtual       := 0
    Local lRet         := .T.
    Local oMessage     := Nil
    ...
    If lRet
        ...
        //Inicializa conexão
        nRet := oServer:Init("", cServer, cUser, cPass, 0, nPort)
        If nRet != 0
            cLog += "004 - Nao foi possivel inicializar o servidor SMTP: " + oServer:GetErrorString(nRet) + CRLF
            lRet := .F.
        EndIf

        If lRet
            //Define o time out
            nRet := oServer:SetSMTPTimeout(nTimeOut)
            If nRet != 0
            ...
            If nRet <> 0
                cLog += "006 - Nao foi possivel conectar no servidor SMTP: " + oServer:GetErrorString(nRet) + CRLF
                lRet := .F.
            EndIf
            ...
            If lRet
                ...
                If nRet <> 0
                    cLog += "007 - Nao foi possivel autenticar no servidor SMTP: " + oServer:GetErrorString(nRet) + CRLF
                    lRet := .F.
                EndIf
                ...
                If lRet
                    ...
                    If nRet <> 0
                        cLog += "008 - Nao foi possivel enviar a mensagem: " + oServer:GetErrorString(nRet) + CRLF
                        lRet := .F.
                    EndIf
                EndIf
                ...
                If nRet <> 0
                    cLog += "009 - Nao foi possivel disconectar do servidor SMTP: " + oServer:GetErrorString(nRet) + CRLF
                EndIf
            EndIf
        EndIf
    EndIf
