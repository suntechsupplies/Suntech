...
User Function ENVLDIGT(_aEmp)
	...
	If lSchedule
		...
		cQuery += " FROM " + RetSqlName("SE1") + " SE1, " + RetSqlName("SED") + " SED (NOLOCK)"
		cQuery += " WHERE 0=0 "
		cQuery += " AND SE1.E1_NATUREZ = SED.ED_CODIGO "			// Ricardo Araujo - Suntech 27/04/2023            
		cQuery += " AND SE1.D_E_L_E_T_ = '' "						// Alteração Realizada para Impedir
